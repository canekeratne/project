...
class myConnect
{
  public:
    ...
    map<int, string> insert(string table_name, vector<int> &IDLIST);
...
myConnect::myConnect(string db_name = "test")
{
  driver = get_driver_instance();
  con = driver->connect("localhost", "root", "db3a53");
...
void myConnect::create_global(string table_name)
{
...
void myConnect::create_local(string table_name)
{
  string q = "DROP TABLE IF EXISTS ";
  q += table_name;
  stmt->execute(q);
...
void myConnect::create_mapping(string table_name)
{
...
void myConnect::insert_mapping(int local, string global)
{
...
vector<string> myConnect::describe(string table_name){
  vector<string> ret_v;
...
void myConnect::print_results(string field, vector<int> loc_ids)
{
  vector<int> g_ids;
  vector<string> v = describe(global_table);
...
bool myConnect::unique(vector<int> v, int k)
{
  bool u;
  for(int i = 0; i < v.size(); i++)
  {
...
map<int, string> myConnect::insert(string table_name, vector<int> &IDLIST)
{
...
int myConnect::local_search(string data)
{
...
vector<int> myConnect::field_search(string field)
{
  vector<int> temp;
  string q = "SELECT local_id FROM ";
  q += field;
...
vector<string> myConnect::split(const string &s, char delim){
  stringstream ss(s);
  string item;
...
bool myConnect::isUnique(string table, string data){
